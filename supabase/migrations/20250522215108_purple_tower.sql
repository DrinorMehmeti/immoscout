/*
  # Create current viewers tracking system

  1. New Tables
    - `current_viewers` table to track users currently viewing properties
  
  2. New Functions
    - `register_property_viewer` to register or update viewer activity
    - `cleanup_stale_viewers` to remove inactive viewers 
    - `count_current_viewers` to get current viewer count for a property
    - `get_property_with_viewer_count` to get property details with viewer count

  3. Security
    - Enable RLS on the new table
    - Add policies to control access to viewer data
    - Create function to clean up stale viewers
*/
...
-- Create a unique index for authenticated viewers
CREATE UNIQUE INDEX IF NOT EXISTS unique_authenticated_viewer 
ON current_viewers(property_id, viewer_id) 
WHERE viewer_id IS NOT NULL;

-- Create a unique index for anonymous viewers
CREATE UNIQUE INDEX IF NOT EXISTS unique_anonymous_viewer 
  ...
  EXCEPTION 
    WHEN unique_violation THEN
      ...
      IF v_viewer_id IS NOT NULL THEN
        ...
        WHERE property_id = p_property_id AND viewer_id = v_viewer_id;
      ELSE
        -- For anonymous users
        UPDATE current_viewers 
        SET last_active = v_now
