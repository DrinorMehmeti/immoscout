/*
  # Property Views Table and Analytics

  1. New Tables
    - `property_views` - Tracks when properties are viewed
      - `id` (uuid, primary key)
  ...
  2. Security
    ...
    - Add policies for view tracking and access control
  
  3. Functions
    - `check_recent_view()` - Prevents duplicate views
    - `get_property_view_stats()` - Analytics function
*/
...
-- Create policies for property_views using DO blocks to check if they exist first
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'property_views' 
    AND policyname = 'Anyone can add a view'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'property_views' 
    AND policyname = 'Property owners can view all views for their properties'
  ) THEN
